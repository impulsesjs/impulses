...
var impulses = function impulses() {
    var _arguments = arguments;

    _classCallCheck(this, impulses);

    function newApi() {
        return new (Function.prototype.bind.apply(_api2.default, [null].concat(Array.prototype.slice.call(arguments))))();
    }

    function newBus() {
        return new (Function.prototype.bind.apply(_bus2.default, [null].concat(Array.prototype.slice.call(arguments))))();
    }

    this.getNewApi = function () {
        return newApi.apply(undefined, _arguments);
    };

    this.getNewBus = function () {
        return newBus.apply(undefined, _arguments);
    };
};

var Impulses = exports.Impulses = new impulses();
...
function ApiClass() {
    ...
    var publicBus = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    var privateBus = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
    ...
    if (publicBus !== null && publicBus.constructor !== _bus2.default) {
        throw TypeError('Public BUS Object is Invalid');
    }

    if (privateBus !== null && privateBus.constructor !== _bus2.default) {
    ...
    var pubBus = publicBus;
    var privBus = privateBus;
    var id = undefined;
    ...
    function initId() {
        if (!id) {
            ...
            return new _md2.default().calculate(JSON.stringify(this) + ('' + Math.random() * 5000));
        }
        return id;
    }
     ...
     */
    function isBus(bus) {
        return bus.constructor === _bus2.default;
    }
     ...
     */
    function registerChannels(targetBus) {
        var channelConfig = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    ...
    function sendMessage(entity, channel, message, bus) {
        /** @type {Bus} */
        var targetBus = bus || pubBus;

        if (isBusSet(targetBus)) {
            var channelObj = targetBus.get(entity, channel);
            if (channelObj !== null) {
    ...
    function reply(message, requestMessage) {
        ...
        if (reply_to !== false) {
            var channel_found = discoverChannel(reply_to.entity, reply_to.name);
     ...
     * @param {Bus} publicBus BUS to be used as public
     */
    this.setPublicBus = function (publicBus) {
        return setPublicBus(publicBus);
    };
     ...
     */
    this.setPrivateBus = function (privateBus) {
        return setPrivateBus(privateBus);
    };
     ...
     * @return {String[]}             Array with registered channels (check bus.register)
     */
    this.registerPublic = function (channelConfig) {
        return registerChannels(pubBus, channelConfig);
    };
     ...
     * @return {String[]}             Array with registered channels (check bus.register)
     */
    this.registerPrivate = function (channelConfig) {
        return registerChannels(privBus, channelConfig);
    };
     ...
     * @return {int|false}      Registered Id or False
     */
    this.sendPublic = function (entityName, channelName, message) {
        return sendMessage(entityName, channelName, message, pubBus);
    };
     ...
     * @return {int|false}      Registered Id or False
     */
    this.sendPrivate = function (entityName, channelName, message) {
        return sendMessage(entityName, channelName, message, privBus);
    };
     ...
     * @return {boolean}
     */
    this.existsPublic = function (entityName, channelName) {
        return !!discovePublicChannel(entityName, channelName);
    };
     ...
     * @return {boolean}
     */
    this.existsPrivate = function (entityName, channelName) {
        return !!discovePrivateChannel(entityName, channelName);
    };
